...
- **近期计划版本**：采用建议后保存任务、时间预算、依据和复查日期；可撤销到上一版本，历史不会被覆盖。
- **方法与复测连接**：可为听、读、写、说、词汇选择一个排查重点，获得预算内练习、完成标准和复测说明；没有明确科目时不根据自由描述猜测错因。
- **效果复查**：区分执行后有效、执行后无效、证据不足和尚未执行；同类方法连续无效时暂停继续采用。
- **学习记录备份**：可导出并在格式校验后恢复记录和计划。备份可能包含个人描述，请勿提交到公开仓库。
